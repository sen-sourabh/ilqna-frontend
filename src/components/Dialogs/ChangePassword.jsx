...
export const ChangePassword = ({ email }) => {
  ...
  const isValid = () => {
    if (
      !functions.isEmpty(oldPassword) ||
      !functions.isEmpty(oldPassword) ||
      !functions.isEmpty(oldPassword)
    ) {
      dispatch(
        prepareSnackbar({ open: true, severity: 'error', message: 'All fields are required.' }),
      );
      setTimeout(() => {
    ...
    if (!Password.isValidPassword(cnewPassword)) {
      dispatch(
        prepareSnackbar({
          open: true,
          severity: 'error',
          message: 'Please enter a valid confirm new password.',
        }),
      );
      setTimeout(() => {
        dispatch(resetSnackbar());
    ...
    if (!Password.isPasswordMatch(newPassword, cnewPassword)) {
      dispatch(
        prepareSnackbar({
          open: true,
          severity: 'error',
          message: 'New password & Confirm password are not matched.',
        }),
      );
      setTimeout(() => {
        dispatch(resetSnackbar());
  ...
  return (
    <Dialog open={isOpen} onClose={handleClosePassword}>
      <DialogTitle>
        Change Password
        <br></br>
        <h6 style={{ margin: 0, fontWeight: 100, fontSize: '12px' }}>
          Password should contain minimum 8 characters that includes capital letter, small letter,
