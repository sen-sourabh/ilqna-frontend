...
export const getGeoLocation = async (ip) => {
    return await axios.get(ENV.STACKGEO_BASE_URL + ip)
    .then((result) => {
        localStorage.setItem('ipLocationData', JSON.stringify({ ipAddress: ip, location: `${result.data.city}, ${result.data.regionName} (${result.data.region}), ${result.data.country} (${result.data.countryCode})` }) );
